...
MaterialProperty::MaterialProperty(Material* material, QWidget* parent /* = 0 */)
{
	m_host = material;
	...
	m_shininessSlider = new FloatSlider(Qt::Horizontal, 0.0f, 1.0f, this);

	m_colorEditSlider = new Vector3DEditSlider("Color", Qt::Horizontal, "R", "G", "B", 0.0, 1.0, 2, this);
...
void MaterialProperty::configSignals()
{
	...
	//material当中的组件发生变化，那么material相关属性应该发生变化
	...
	//设置当material相关属性发生变化，相应组件也应该发生变化
...
void MaterialProperty::hostDestroyed(QObject* host)
{
	delete this;
}
...
void TextureProperty::resizeEvent(QResizeEvent* event)
{
...
{
	//挂钩的texture相关的属性变化，对应的property也发生变化
	connect(m_host, SIGNAL(destroyed(QObject*)), this, SLOT(hostDestroyed(QObject*)));
	...
	//挂钩的texture设置为false，对应的property设置为不可见
	connect(m_host, SIGNAL(toggled(bool)), this, SLOT(setEnabled(bool)));
}
