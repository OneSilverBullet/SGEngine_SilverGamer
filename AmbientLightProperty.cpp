...
AmbientLightProperty::AmbientLightProperty(AmbientLight* light, QWidget* parent /* = 0 */)
{
	m_host = light;

	m_enabledCheckBox = new QCheckBox("Enabled", this);
	m_intensityLable = new QLabel("Intensity", this);
	m_intensityEdit = new FloatEdit(0.0f, inf, 2, this);
	m_intensitySlider = new FloatSlider(Qt::Horizontal, 0.0, 1.0, this);
...
void AmbientLightProperty::configLayout()
{
	...
	mainLayout->addWidget(m_intensityLable, 1, 0);
...
void AmbientLightProperty::configSignals()
{
	...
	connect(m_enabledCheckBox, SIGNAL(toggled(bool)), m_intensityLable, SLOT(setEnabled(bool)));
...
void AmbientLightProperty::hostDestroyed(QObject* host)
{
	delete this;
}